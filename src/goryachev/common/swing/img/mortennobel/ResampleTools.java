...
public class ResampleTools
{
	...
	public static void ints2bytes(int[] in, byte[] out, int index1, int index2, int index3)
	{
		for(int i=0; i<in.length; i++)
		{
			int ix = i * 3;
			int value = in[i];
			out[ix + index1] = (byte)value;
			value = value >> 8;
			out[ix + index2] = (byte)value;
			value = value >> 8;
			out[ix + index3] = (byte)value;
		}
	}
	...
	{
		BufferedImage im = new BufferedImage(src.getWidth(), src.getHeight(), bufImgType);
